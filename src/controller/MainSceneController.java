...
//        
    ...
    public void mouseClick() {
        ...
        if(item.equals(items[0])){
        	FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/fxml/textArea.fxml"));
            Parent root = null;
            try {
...
//            listView.getItems().clear();
        	...
        	FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/fxml/ImageTextScene.fxml"));
            Parent root = null;
            try {
                root = fxmlLoader.load();
            } catch (IOException e) {
    ...
    private void search_action() {
    	...
    	if(searchString.equals("")) {
    		...
    		for(String str : nameSelectedList) {
        		if(removeAccent(str.strip()).toLowerCase().startsWith(removeAccent(searchString).strip().toLowerCase()) == true && nameList.contains(str) == false) {
        			nameList.add(str);
            		found = true;
        		}
        	}
